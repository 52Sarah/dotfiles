...
# if [[ -z "$_DOT_BOOTSTRAP_PROFILE_MTIME" ]] || (( $(stat -L -f '%m' ~/.zprofile) > _DOT_BOOTSTRAP_PROFILE_MTIME )); then
...
.tick-bootstrap-profile() { .tick -s '.sh_bootstrap_profile' $@; }
.tick-bootstrap-profile "[START-FILE] (\$\$=$$), mtime=$(stat -L -f '%m' ~/.sh_bootstrap_profile)" #, \$PATH=[$PATH], \$PS1=[$PS1])"

bootstrap-profile-wrapper() {

  .tick-bootstrap-profile "[START-WRAPPER] (\$\$=$$)" #, \$PATH=[$PATH], \$PS1=[$PS1])"
  ...
  #
  .setup-homebrew() {
    .tick-bootstrap-profile '[start] .setup-homebrew'
    if [[ ! -x /usr/local/bin/brew ]]; then
      .tick-bootstrap-profile "... homebrew not installed"
      .tick-bootstrap-profile "... execute: /bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\""
      return 1
    fi

    ((_DEBUG)) && .tick-bootstrap-profile "... PATH before 'brew shellenv': [$PATH]"
    eval "$(/opt/homebrew/bin/brew shellenv 2>/dev/null)"
    ((_DEBUG)) && .tick-bootstrap-profile "... PATH after  'brew shellenv': [$PATH]"
    if [[ ! -d "$HOMEBREW_PREFIX" ]]; then
      .tick-bootstrap-profile "... homebrew 'shellenv' did not properly set \$HOMEBREW_PREFIX"
      return 1
    fi
    # Let path-prepend de-dupe the /usr/local/... paths.
    path-prepend PATH /usr/local/sbin
    path-prepend PATH /usr/local/bin
    .tick-bootstrap-profile "... \$HOMEBREW_PREFIX=$HOMEBREW_PREFIX"

    alias bs='qeval brew services'
    
    _QUIET=1 safe-source /usr/local/etc/bash_completion.d/brew && .tick-bootstrap-profile '... loaded brew completion' || .tick-bootstrap-profile '!!! failed to load brew completion'

    local gnu_getopt_home="$HOMEBREW_PREFIX/opt/gnu-getopt"
    if [[ -e "$gnu_getopt_home" ]]; then
      path-prepend PATH "$gnu_getopt_home/bin"
      .tick-bootstrap-profile "... prepended $gnu_getopt_home/bin to PATH"
    fi

    local openssl_home="$HOMEBREW_PREFIX/opt/openssl@1.1"
    if [[ -e "$openssl_home" ]]; then
      path-prepend PATH "$openssl_home/bin"
      .tick-bootstrap-profile "... prepended $openssl_home/bin to PATH"
    fi

    .tick-bootstrap-profile "[end] .setup-homebrew, PATH=$PATH"
  }
  ...
  #
  .setup-iterm() {
    .tick-bootstrap-profile '[start] .setup-iterm'
    [[ "$TERM_PROGRAM" != "iTerm.app" ]] && .tick-bootstrap-profile "... iTerm2 not installed" && return 1
    ...
    .tick-bootstrap-profile "[end] .setup-iterm, ITERM_PROFILE=$ITERM_PROFILE"
  }
  ...
  #
  .setup-git() {
    .tick-bootstrap-profile '[start] .setup-git'
    ! is-defined git && .tick-bootstrap-profile "[end] .setup-git: git not installed" && return 0

    .tick-bootstrap-profile "... using $(git --version)"
  ...
  # %s  - subject line
    ...
    .tick-bootstrap-profile '... checking for git-flow'
    if is-defined git-flow; then
      # https://github.com/aleksandr-m/gitflow-maven-plugin

      .tick-bootstrap-profile "... checking for ~/.git-flow-completion"
      _QUIET=1 safe-source ~/.git-flow-completion
      complete -p | grep -E -q 'git-flow$' && .tick-bootstrap-profile "... loaded git-flow cli completion" || .tick-bootstrap-profile "... not using git-flow completion"
    ...
    .setup-git-prompt() {
      .tick-bootstrap-profile "[start] .setup-git-prompt, PS1=[$PS1]"
      ...
      .tick-bootstrap-profile "[end] .setup-git-prompt, PS1=[$PS1]"
    }
    .setup-git-prompt 

    .tick-bootstrap-profile '[end] .setup-git'
  }
  ...
  #
  .setup-java-sdkman() {
    .tick-bootstrap-profile '[start] .setup-java-sdkman'
    if [[ ! -e "$HOME/.sdkman/bin/sdkman-init.sh" ]]; then
      .tick-bootstrap-profile "[end] .setup-java-sdkman, SDKMAN not installed"
      return 1
    fi
    ...
    if [[ "$(type -t sdk)" == "function" ]]; then
      .tick-bootstrap-profile 'sdkman already initialized'
    else
      .tick-bootstrap-profile '... initializing sdkman'
      . "$SDKMAN_DIR/bin/sdkman-init.sh"
      path-prepend "$SDKMAN_DIR/bin"
      .tick-bootstrap-profile "... initialized sdkman"
    fi
    # .tick-bootstrap-profile -e 'echo "... using $(sdkman version)"'
    # .tick-bootstrap-profile -e 'echo "... $ which javac: $(2>&1 which javac)"'
    # .tick-bootstrap-profile -e 'echo "... $ javac -version: $(2>&1 javac -version)"'
    
    if [[ -z "$JAVA_HOME" ]]; then
      .tick-bootstrap-profile "sdkman left a blank JAVA_HOME"
    elif [[ ! -d "$JAVA_HOME" ]]; then
      .tick-bootstrap-profile "sdkman left a non-directory JAVA_HOME: $JAVA_HOME"
    elif [[ ! -d "$JAVA_HOME/bin" ]]; then
      .tick-bootstrap-profile "sdkman non-directory JAVA_HOME/bin: $JAVA_HOME/bin"
    fi
    ...
    .tick-bootstrap-profile -e tilde-compress "[end] .setup-java-sdkman, JAVA_HOME=[$JAVA_HOME], PATH=$PATH"
  }
  ...
  #
  .setup-java-jenv() {
    .tick-bootstrap-profile '[start] .setup-java-jenv'
    ! is-defined jenv && .tick-bootstrap-profile "[end] .setup-java-jenv, jenv not installed" && return 1

    if [[ "$(type -t jenv)" == "function" ]]; then
      .tick-bootstrap-profile 'jenv already initialized'
    else
      .tick-bootstrap-profile '... initializing jenv'
      eval "$(jenv init --no-rehash -)"
      path-prepend "$HOME/.jenv/bin"
      .tick-bootstrap-profile "... initialized jenv"
    fi
    # .tick-bootstrap-profile -e 'echo "... using $(jenv --version)"'
    # .tick-bootstrap-profile -e 'echo "... using java $(jenv version)"'
    # .tick-bootstrap-profile -e 'echo "... $ which javac: $(2>&1 which javac)"'
    # .tick-bootstrap-profile -e 'echo "... $ javac -version: $(2>&1 javac -version)"'
    
    local javahome="$(jenv javahome)"
    if [[ -z "$javahome" ]]; then
      .tick-bootstrap-profile "jenv reports a blank JAVA_HOME"
    elif [[ ! -d "$javahome" ]]; then
      .tick-bootstrap-profile "jenv reports a non-directory JAVA_HOME: $javahome"
    elif [[ ! -d "$javahome/bin" ]]; then
      .tick-bootstrap-profile "jenv non-directory JAVA_HOME/bin: $javahome/bin"
    else
      export JAVA_HOME="$javahome"
    fi

    .tick-bootstrap-profile -e tilde-compress "[end] .setup-java-jenv, JAVA_HOME=[$JAVA_HOME], PATH=$PATH"
  }
  ...
  #
  .setup-pg() {
    .tick-bootstrap-profile '[start] .setup-pg'
    export HOMEBREW_POSTGRESQL_SERVICE="$(readlink /usr/local/opt/postgresql)"
    if [[ -z "$HOMEBREW_POSTGRESQL_SERVICE" ]]; then
      .tick-bootstrap-profile '[end] .setup-pg, no /usr/local/opt/postgresql; pg not installed'
      return 1
    fi
    ...
    .tick-bootstrap-profile '[end] .setup-pg, PATH=$PATH"'
  }
  ...
  .setup-schemaspy() {
    if [[ ! -e "$HOME/lib/schemaspy.jar" ]]; then
      .tick-bootstrap-profile "[end] .setup-schemaspy, schemaspy.jar not installed in ~/lib"
      return 1
    fi
  ...
  #
  .setup-vbox() {
    ! is-defined VBoxManage && .tick-bootstrap-profile '[end] .setup-vbox, VirtualBox not installed' && return 1
  ...
  .source-extra-start-files() {
    .tick-bootstrap-profile "[start] .source-extra-start-files ($1)"
    if glob-path-exists ~/$1.*; then
      for f in ~/$1.*; do
        .tick-bootstrap-profile "... sourcing $f"
        . $f
      done
    fi
    .tick-bootstrap-profile "[end] .source-extra-start-files ($1)"
  }
  # .source-extra-start-files
...
export _DOT_BOOTSTRAP_PROFILE_MTIME="$(stat -L -f '%m' ~/.zprofile)"
.tick-bootstrap-profile "[END-FILE] (\$\$=$$), mtime=$_DOT_BOOTSTRAP_PROFILE_MTIME" #, \$PATH=[$PATH])"
