...
# Shell-agnostic bootstrap functions and aliases only.

# if [[ -z "$_DOT_SHBOOTSTRAP_MTIME" ]] || (( $(stat -L -f '%m' ~/.sh_bootstrap) > _DOT_SHBOOTSTRAP_MTIME )); then
  ...
  export _DOT_SHBOOTSTRAP_MTIME="$(stat -L -f '%m' ~/.sh_bootstrap)"
# fi
